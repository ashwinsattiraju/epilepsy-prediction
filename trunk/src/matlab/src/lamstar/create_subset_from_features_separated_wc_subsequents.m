function [fraction remaining] = create_subset_from_features_separated_wc_subsequents(features, num_samples_max)
    ...
    for i=1:total_words
        ...
        if (total_words_in_fraction_attack > 0 && word_class==1)
            ...
            for dec=1:8
                ...
                fraction.entropy_wc_whole(eeg_num). decomposition_num(dec).data(next_window_fraction_index(eeg_num)) = features.entropy_wc_whole(eeg_num).decomposition_num(dec).data(window_num);
                data = normalize(features.entropy_wc_thirds(eeg_num).decomposition_num(dec).data(window_num));
                fraction.entropy_wc_thirds(eeg_num).decomposition_num(dec).data(next_window_fraction_index(eeg_num)) = data;
                data = normalize(features.entropy_wc_fifths(eeg_num).decomposition_num(dec).data(window_num));
                ...
                fraction.kolmogorov_wc_whole(eeg_num).decomposition_num(dec).data(next_window_fraction_index(eeg_num)) = features.kolmogorov_wc_whole(eeg_num).decomposition_num(dec). data(window_num);
                data = normalize(features.kolmogorov_wc_thirds(eeg_num).decomposition_num(dec).data(window_num));
                fraction.kolmogorov_wc_thirds(eeg_num).decomposition_num(dec).data(next_window_fraction_index(eeg_num)) = data;
                data = normalize(features.kolmogorov_wc_fifths(eeg_num).decomposition_num(dec).data(window_num));
                ...
                fraction.mean_energy_wc_whole(eeg_num). decomposition_num(dec).data(next_window_fraction_index(eeg_num)) = features.mean_energy_wc_whole(eeg_num).decomposition_num(dec). data(window_num);
                data = normalize(features.mean_energy_wc_thirds(eeg_num).decomposition_num(dec).data(window_num));
                fraction.mean_energy_wc_thirds(eeg_num).decomposition_num(dec).data(next_window_fraction_index(eeg_num)) = data;
                data = normalize(features.mean_energy_wc_fifths(eeg_num).decomposition_num(dec).data(window_num));
        ...
        % the remaining words are for the remaining structure
            ...
            for dec=1:8
                ...
                remaining.entropy_wc_whole(eeg_num). decomposition_num(dec).data(next_window_remaining_index(eeg_num))    = features.entropy_wc_whole(eeg_num).     decomposition_num(dec). data(window_num);
                data = normalize(features.entropy_wc_thirds(eeg_num).decomposition_num(dec).data(window_num));
                ...
                % copy data structures / 5
                % [kolmogorov of wc whole/thirds/fifths, decomposition by decomposition]
                remaining.kolmogorov_wc_whole(eeg_num). decomposition_num(dec).data(next_window_remaining_index(eeg_num)) = features.kolmogorov_wc_whole(eeg_num).  decomposition_num(dec). data(window_num);
                data = normalize(features.kolmogorov_wc_thirds(eeg_num).decomposition_num(dec).data(window_num));
                ...
                % copy data structures / 6
                % [mean energy of wc whole/thirds/fifths decomposition by decomposition]
                remaining.mean_energy_wc_whole(eeg_num). decomposition_num(dec).data(next_window_remaining_index(eeg_num)) = features.mean_energy_wc_whole(eeg_num).decomposition_num(dec). data(window_num);
                data = normalize(features.mean_energy_wc_thirds(eeg_num).decomposition_num(dec).data(window_num));
